...
namespace ts::detail::quarantined
{
...
#define NUM_SUPPORTED_TEXTURES 80
    ...
    TS_PhysicsObject::TS_PhysicsObject(btCollisionShape* s, float mass, bool isKinematic, bool isTrigger,
                                       const btVector3& initPos, const btQuaternion& initRot)
    {
        this->cshape = s;
        this->cobj = nullptr;
        this->rbody = nullptr;
        ...
        if (isKinematic || isTrigger)
        {
            ...
            if (isTrigger)
                this->cobj->setCollisionFlags(
                        this->cobj->getCollisionFlags() | btCollisionObject::CF_NO_CONTACT_RESPONSE);
...
// convenient typedefs for collision events
    ...
    TS_VmaCreateBuffer(vk::DeviceSize size, vk::Flags<vk::BufferUsageFlagBits> usage,
    ...
    std::pair<vk::Image, vma::Allocation>
    TS_VmaCreateImage(uint32_t width, uint32_t height, vk::Format fmt, vk::ImageTiling tiling,
...
// normalized texture coordinates along the u and v axes
    ...
    int TS_VkLoadTexture(const char* img)
    {
        ...
        if (!txtInds.count(std::string(img)))
        {
            ...
            std::pair<vk::Buffer, vma::Allocation> pixelStaging = TS_VmaCreateBuffer(pixels.size() * sizeof(uint8_t),
                                                                                     vk::BufferUsageFlagBits::eTransferSrc,
                                                                                     vk::MemoryPropertyFlagBits::eHostVisible |
                                                                                     vk::MemoryPropertyFlagBits::eHostCoherent,
                                                                                     vma::AllocationCreateFlagBits::eMapped);
            memcpy(al.getAllocationInfo(pixelStaging.second).pMappedData, (void*) pixels.data(),
                   pixels.size() * sizeof(uint8_t));
            std::pair<vk::Image, vma::Allocation> pixelImg = TS_VmaCreateImage(wdth, hght, vk::Format::eR8G8B8A8Unorm,
                                                                               vk::ImageTiling::eOptimal,
                                                                               vk::ImageUsageFlagBits::eTransferDst |
                                                                               vk::ImageUsageFlagBits::eSampled,
                                                                               vk::MemoryPropertyFlagBits::eDeviceLocal);
            TS_VkTransitionImageLayout(pixelImg.first, vk::ImageLayout::eUndefined,
                                       vk::ImageLayout::eTransferDstOptimal);
            TS_VkCopyBufferToImage(pixelStaging.first, pixelImg.first, wdth, hght);
            TS_VkTransitionImageLayout(pixelImg.first, vk::ImageLayout::eTransferDstOptimal,
                                       vk::ImageLayout::eShaderReadOnlyOptimal);
            vk::ImageView v = TS_VkCreateImageView(pixelImg.first, vk::Format::eR8G8B8A8Unorm,
    ...
    void TS_VkCmdDrawSprite(const char* img, float r, float g, float b, float a, int rx, int ry, int rw, int rh, int cw,
                            int ch, int ci, int cj, float px, float py, float sx, float sy)
    {
    ...
    void TS_VkCmdClearColorImage(float r, float g, float b, float a)
    {
        ...
        cmdbufs[frameIndex].clearColorImage(swapchainImages[frameIndex], vk::ImageLayout::eGeneral, &clearColor, 1U,
                                            &imageRange);
    }
    ...
    {
        // copy data
        memcpy(al.getAllocationInfo(vertexStaging.second).pMappedData, (void*) vertices.data(),
               vertices.size() * sizeof(TS_Vertex));
        memcpy(al.getAllocationInfo(indexStaging.second).pMappedData, (void*) indices.data(),
        ...
        cmdbufs[frameIndex].bindDescriptorSets(vk::PipelineBindPoint::eGraphics, trianglePipelineLayout, 0, 1, &dscSet,
                                               0, 0);
    ...
    void TS_VkQueueSubmit()
    {
        ...
        vk::SubmitInfo submitInfo(1, &imageAvailableSemaphore, &waitDestStageMask, 1, &cmdbufs[frameIndex], 1,
    ...
    void TS_VmaCreateBuffers()
    {
        ...
        indexStaging = TS_VmaCreateBuffer(defaultBufferSize, vk::BufferUsageFlagBits::eTransferSrc,
                                          ...
                                          vk::MemoryPropertyFlagBits::eHostVisible,
                                          vma::AllocationCreateFlagBits::eMapped);
        vertexBuffer = TS_VmaCreateBuffer(defaultBufferSize, vk::BufferUsageFlagBits::eTransferDst |
    ...
    void TS_VkCreateImageViews()
    {
        for (int i = 0; i < swapchainImages.size(); ++i)
        {
            swapchainImageViews.push_back(
                    TS_VkCreateImageView(swapchainImages[i], surfaceFormat.format, vk::ImageAspectFlagBits::eColor));
        }
    }
    ...
    void TS_VkSetupDepthStencil()
    {
        ...
        depthImageView = TS_VkCreateImageView(depthImage.first, vk::Format::eD32SfloatS8Uint,
    ...
    void TS_VkCreateDescriptorSet()
    {
        ...
        vk::DescriptorBindingFlags layoutBindingFlags[] = {vk::DescriptorBindingFlags(),
    ...
    {
        TS_PhysicsObject* g = new TS_PhysicsObject(new btBoxShape(btVector3(hy, hx, hz)), m, isKinematic, false,
                                                   btVector3(px, py, pz));
    ...
    {
        TS_PhysicsObject* g = new TS_PhysicsObject(new btBoxShape(btVector3(hy, hx, hz)), 0.0f, false, false,
                                                   btVector3(px, py, pz));
        physicsObjectsById[id] = g;
        idsByPtr[static_cast<void*>(g->rbody)] = id;
    }
    ...
    {
        TS_PhysicsObject* g = new TS_PhysicsObject(new btBoxShape(btVector3(hy, hx, hz)), 1.0f, false, true,
                                                   btVector3(px, py, pz));
        physicsObjectsById[id] = g;
    ...
    void TS_Init(const char* ttl, int wdth, int hght)
    {
        ...
        {
            std::cerr << "Failed to initialise audio mixer properly. All sounds may not play correctly." << std::endl
                      << TS_SDLGetError() << std::endl;
        }

        if (Mix_OpenAudio(22050, MIX_DEFAULT_FORMAT, 2, 1024) != 0)
        {
            std::cerr << "No audio device available, sounds and music will not play." << std::endl << TS_SDLGetError()
                      << std::endl;
            Mix_CloseAudio();
        }
        ...
        window_width = wdth;
        window_height = hght;
        win = SDL_CreateWindow(ttl, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, wdth, hght,
    ...
    void TS_PlaySound(const char* sound_file, int loops, int ticks)
    {
        ...
        if (sample == NULL)
        {
            std::cerr << "Could not load sound file: " << std::string(sound_file) << std::endl << TS_SDLGetError()
                      << std::endl;
            return;
        }
